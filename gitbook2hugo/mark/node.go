...
func DumpNode(node Node, level int) string {
	...
	s = append(s, strings.Repeat("  ", level)+node.NodeName())
	for _, child := range node.Children() {
		s = append(s, DumpNode(child, level+1))
	}
...
func CollectNode(node Node, doCollect func(node Node) bool) (nodes []Node) {
	for _, child := range node.Children() {
		...
		cnodes := CollectNode(child, doCollect)
...
func (t NodeType) Dump(level int) string {
	...
	s = append(s, strings.Repeat("  ", level)+t.NodeName())
	for _, child := range t.Children() {
		s = append(s, strings.Repeat("  ", level+1)+child.Dump(level+1))
	}
	return strings.Join(s, "\n")
}
