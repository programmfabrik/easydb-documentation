...
| **Status request repitions** | <ul><li> _mandatory_ </li><li>Maxmimum number of requests of the status of the analyzing of an image in the external service, before the keywording for the image is discarded</li><li>Together with the option **Status request delay** you can control the total waiting time before a yet not analyzed image is discarded</li></ul> |
| **Status request delay** | <ul><li> _mandatory_ </li><li>Minimal delay between repeated status requests for an image</li><li>After the analyzing status of an image was requested, and the analyzing process is not finished (status is neither complete nor there was an error), the plugin will wait for this time span, before the status will be requested again</li><li>Together with the option **Status request repitions** you can control the total waiting time before a yet not analyzed image is discarded</li><li>Depending on the time the external service needs to repsond to a status request, you can (roughly) estimate the total time after which the plugin will no longer request the status of an image, by multiplying **Status request repitions** with **Status request delay**</li></ul> |
