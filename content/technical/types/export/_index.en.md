...
| &#8614; `merge_linked_objects` | which linked objects are exported in XML exports (string, rw, optional) - its value can be: <ul><li>**"none"**: don't include any linked objects (default value)<li>**"not_in_main_search"**: only load and export linked objects that are not in the main search (data model option)<li>**"not_in_main_search_and_reverse"**: like **"not_in_main_search"**, but also include reverse nested main objects </ul> |
