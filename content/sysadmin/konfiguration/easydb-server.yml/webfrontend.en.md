...
| &#8193;`level`                          | String  | Overwrites the highest permitted database rights level. Allowed values are: **current** (the current datamodel is visible), **development** (*current* plus the dev datamodel is visible and changeable), **commit** (*development* plus the dev datamodel can be committed as the new version of the current datamodel).  | - |
