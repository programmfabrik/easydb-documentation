...
The <i class = "fa fa-share"> </i> button in the detail view can be used to create a link to the **record**. The weblink, if configured, leads to the record in easydb. The link displays the most recent version of the record. Older versions can also be accessed via this link when indicated with **version/< number >**. Further options for [Deep Links](https://docs.easydb.de/en/technical/api/objects/objects.html) are listed in the technical part. No special permissions can be configured for this type of link. 
