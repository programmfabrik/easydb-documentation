...
In the large area there is the possibility to upload further files with `+ files` {.button} and` + directory` {.button}. `+ Directory` {.button} is currently only offered in*Google Chrome*and*Apple Safari*. You can also drag additional files into this area by drag & drop.

Click `Next ...` {.button} to open the editor.
...
On the left you will see the list of the new data sets that are created when you click on `Save` {.button}. At the top there is *Template*. The entries here refer to all data records that you create.
...
After the input, the process can be terminated with `Save` {.button}. If there is a required field, this must be completed before the operation can be saved. If the required field is multilingual, only one of the fields must be filled to complete the process.
...
When uploading new records, easydb checks whether the record is already included. You can then decide whether you want this record `skip` {.button} or` import` {.button}.
...
If you agree with `Import`{.button} to accept the duplicate, you can proceed with the creation of the data record as usual.
