...
On success, an e-mail is sent out to user identified by `forgot` containing a link to set a new password.

If no active e-mail is found, but there is an e-mail to be confirmed, the confirmation request will be
resent to the user. If no valid e-mail is found, an error is returned.
